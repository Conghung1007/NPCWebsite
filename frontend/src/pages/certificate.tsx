...
export function CertificatePage({ attemptId }: CertificatePageProps) {
  ...
  const { data: attempt, isLoading: attemptLoading } = useQuery<AttemptWithUser>({
  ...
  const { data: detailsPayload, isLoading: questionsLoading } = useQuery<any>({
  ...
  const scoringSnapshot = Array.isArray(detailsPayload)
    ? null
    : detailsPayload?.scoringSnapshot || null;
  ...
  // Prefer identity tied to the attempt (not whoever is currently logged in).
  // Demo / anonymous: never require login — use generic examinee name.
  const certificateUser = attempt?.user
    ?? (attempt && (!attempt.userId || exam?.isDemo)
      ? { id: "anonymous", fullName: "Thí sinh", username: "Thí sinh" }
      : null);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8">
          <p className="text-xl text-gray-600 mb-2">Không xem được chứng nhận</p>
          <p className="text-gray-600 mb-6">
            Bạn cần đăng nhập bằng tài khoản đã làm bài để xem chứng nhận đề chính thức.
          </p>
          <Button onClick={() => setLocation(`/login?redirect=/certificate/${attemptId}`)} className="mt-2">
            Đăng nhập
          </Button>
        </div>
      </div>
  ...
  if (useSections) {
    ...
    sectionGroups = sourceSections.map((section: any) => {
    ...
    }).filter((section: any) => section.totalQuestions > 0);
  }

  const totalPoints = sectionGroups.reduce((sum, section) => sum + section.totalPoints, 0) ||
    questionsWithAnswers.reduce((sum: number, item: any) => sum + calculateTotalPoints(item), 0);
  const earnedPoints = sectionGroups.reduce((sum, section) => sum + section.earnedPoints, 0) ||
    questionsWithAnswers.reduce((sum: number, item: any) => sum + calculateEarnedPoints(item), 0);
  
  const examPassingScore =
    snapshotExamPassing != null ? snapshotExamPassing : (exam as any).passingScore;
  ...
  const examDate = new Date(attempt.completedAt || attempt.startedAt || Date.now());
