...
export function QuestionBankManager() {
  ...
  const handleDescriptionImageUpload = async (file: File) => {
    ...
    try {
      ...
      form.setValue(`questions.0.descriptionImageUrls`, [...currentUrls, uploadedUrl]);
  ...
  const handleRemoveOption = (questionIndex: number, optionIndex: number) => {
    ...
    if (updatedQuestions[questionIndex].options.length > 2) {
      const removed = updatedQuestions[questionIndex].options[optionIndex];
      if (removed && typeof removed === "object") {
        const urls = [
          (removed as any).imageUrl,
          ...((removed as any).imageUrls || []),
        ];
        void cleanupTempMediaUrls(urls, "qbank");
      }
      updatedQuestions[questionIndex] = {
        ...updatedQuestions[questionIndex],
  ...
  const handleRemoveQuestion = (index: number) => {
    ...
    if (currentQuestions.length > 1) {
      const removed = currentQuestions[index];
      const urls: string[] = [];
      if (removed) {
        if (removed.audioUrl) urls.push(removed.audioUrl);
        if (removed.imageUrl) urls.push(removed.imageUrl);
        (removed.imageUrls || []).forEach((u) => urls.push(u));
        if (Array.isArray(removed.options)) {
          removed.options.forEach((opt: any) => {
            if (typeof opt === "object") {
              if (opt.imageUrl) urls.push(opt.imageUrl);
              (opt.imageUrls || []).forEach((u: string) => urls.push(u));
            }
          });
        }
      }
  ...
  const cancelForm = async () => {
    ...
    const pushIfTemp = (url?: string | null) => {
      if (url && isTempMediaUrl(url)) urls.push(url);
    };

    formData.questions?.forEach((question) => {
      pushIfTemp(question.audioUrl);
      pushIfTemp(question.imageUrl);
      pushIfTemp((question as any).descriptionAudioUrl);
      (question.imageUrls || []).forEach(pushIfTemp);
      ((question as any).descriptionImageUrls || []).forEach(pushIfTemp);
      ...
      if (Array.isArray(question.options)) {
        question.options.forEach((option: any) => {
          if (typeof option === "string") {
            ...
            pushIfTemp(option.imageUrl);
            (option.imageUrls || []).forEach(pushIfTemp);
          }
        });
      }
    });
                        ...
                        {(form.watch(`questions.0.descriptionImageUrls`) || []).map((url, idx) => (
                          <div key={idx} className="relative group">
                            ...
                            <button
                              ...
                              onClick={() => {
                                const currentUrls = form.getValues(`questions.0.descriptionImageUrls`) || [];
                                const removed = currentUrls[idx];
                                cleanupTempMediaUrl(removed, "qbank");
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            cleanupTempMediaUrl(
                              form.getValues(`questions.0.descriptionAudioUrl`),
                              "qbank",
                            );
                            form.setValue(`questions.0.descriptionAudioUrl`, "");
                          }}
                          className="text-red-600 hover:text-red-700"
                        >
                          <X className="w-4 h-4" />
                              ...
                              {(form.watch(`questions.${questionIndex}.imageUrls`) || []).map((url, idx) => (
                                <div key={idx} className="relative group">
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      const currentUrls = form.getValues(`questions.${questionIndex}.imageUrls`) || [];
                                      cleanupTempMediaUrl(currentUrls[idx], "qbank");
                                      const newUrls = currentUrls.filter((_, i) => i !== idx);
                                        ...
                                        <div key={idx} className="relative group">
                                          ...
                                          <button
                                            ...
                                            onClick={() => {
                                              ...
                                              cleanupTempMediaUrl(currentImageUrls[idx], "qbank");
