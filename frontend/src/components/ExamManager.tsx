...
import { Plus, Edit, Trash2, FileText, Clock, Users, Trophy } from "lucide-react";
...
export function ExamManager() {
  ...
  // Fetch all exams for admin (including inactive)
  ...
  const { data: attemptCounts = {} } = useQuery<Record<string, number>>({
    queryKey: examKeys.attemptCounts,
  });
  ...
  const deleteExamMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: examKeys.attemptCounts });
      refetchExams();
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Tạo và quản lý các bài thi trực tuyến (gồm cả đề đang tắt)
              </CardDescription>
            </div>
            <Button 
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        <Button
                          ...
                          title="Xem điểm và danh sách lượt thi"
                          data-testid={`button-view-attempts-${exam.id}`}
                          className="gap-1"
                        >
                          <Trophy className="w-4 h-4" />
                          Xem điểm
                          {(attemptCounts[exam.id] || 0) > 0 && (
                            <Badge variant="secondary" className="ml-1 px-1.5 py-0 text-[10px]">
                              {attemptCounts[exam.id]}
                            </Badge>
                          )}
                        </Button>
                        <Button
